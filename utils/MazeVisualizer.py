...
class MazeVisualizer:
    ...
    def set_images_and_position(self):
        self.images_with_height_and_width = {
            ...
            self.frozen[0] if self.freezed else self.freeze[0]:
            {
                'width': self.win.width // 64 * 63,
            ...
            self.corrector.ptr:
            {
                'width': self.slider_x * self.slider.width // 100 + self.win.width // 128 * 119,
                'height': self.win.height // 64 * 59 + self.play[1]//2 - self.corrector.height//2 + self.slider.height//2
            },
        }
    ...
    def mouse_hook(self, cos, x, y, args):
        ...
        if x_start < x <= x_end:
            for name, reach_y in self.blocks.items():
                if reach_y['start'] + self.segment_height / 2 <= y <= reach_y['end']:
    ...
    def is_key(self, key: str, code: int):
